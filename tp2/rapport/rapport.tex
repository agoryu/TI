...
% Set language to english
  ...
  % -> Les trois premières colonnes de la matrice M représente la matrice intrinsèque.
  %    La matrice intrinsèque ne change pas avec le déplacement de la caméra ou de l'objet dans la scéne.
  %    Quand on change les 3 premières colonnes, ni l'objet, ni la camera ne bouge, c'est les caractéristique 
  %    (pour les -360), la position (pour 80 et 60), du capteur qui change.
  %
  % -> 80 et 60 représente l'origine du capteur
  %    c'est pas un rotation, en gros la surface reste paralléle a la face du cube, mais il bouge assez pour 
  %    voir un autre point de vue du cube.
  ...
  % La rotation n'a pas d'effet car ce n'est pas un rotation comme dit dans le commentaire plus haut.
  % Avec un vrai rotation de la camera la grille drevrait apparaitre déformé par la perceptif (point de fuite).
  ...
  des paramètres qui dépendent de la position de la caméra dans l'espace de travail, comme la rotation % position de la camera dans l'environement*
  ...
  %TODO y'a un prob dans le sujet il dise que la matrice final est de dimension 4x4
  \begin{itemize}
